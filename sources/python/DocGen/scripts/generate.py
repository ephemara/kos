...
Usage:
    python sources/python/DocGen/scripts/generate.py
    python sources/python/DocGen/scripts/generate.py --path crates/k-os-engine
    python sources/python/DocGen/scripts/generate.py --dry-run --provider gemini
    python sources/python/DocGen/scripts/generate.py --watch --concurrency 5
"""
...
def initialize_services(root_path: Path, config: dict, args: argparse.Namespace):
    ...
    # Initialize LanceDB
    lance_db_path = root_path / "sources/python" / "DocGen" / ".lancedb"
    lance_db = LanceDBManager(db_path=lance_db_path)
    logger.info(f"✓ LanceDB initialized at {lance_db_path}")
    
    # Initialize change detector
    cache_dir = root_path / "sources/python" / "DocGen" / ".cache"
...
def main():
    ...
    try:
        ...
        # Assume script is in sources/python/DocGen/scripts/
        ...
        # Load configuration
        config_path = root_path / "sources/python" / "DocGen" / "docgen_config.json"
        ...
        cost_log_path = root_path / "sources/python" / "DocGen" / "cost_log.json"
