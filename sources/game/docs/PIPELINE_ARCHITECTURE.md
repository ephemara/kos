...
Build `sources/game` as a standalone game engine product surface with data-driven orchestration, while reusing existing K_OS strategic owners.
...
For the intended full game-product crate map, see `sources/game/docs/CRATE_ARCHITECTURE.md`.
...
- resolves pipeline bootstrap config from environment data:
  ...
  - `KOS_GAME_PIPELINE_MANIFEST` (default `${workspace_root}/sources/game/config/pipeline.toml`)
...
Current data sources in `sources/game/assets`:
