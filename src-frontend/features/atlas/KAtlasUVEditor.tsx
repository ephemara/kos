...
import { KAtlasUVTransform, UVTransformMode } from './KAtlasUVTransform';
import { Brush, Hand, Zap, Maximize2, Move, RotateCw, Maximize, MousePointer2 } from 'lucide-react';
...
export default function KAtlasUVEditor({ meshes, selectedIds, syncSelection, onSelectionChange, version }: KAtlasUVEditorProps) {
    ...
    const transformEngine = useRef(new KAtlasUVTransform());
    ...
    // Auto-frame ONLY on initial mesh load
    useEffect(() => {
        if (selectedIds.length > 0) {
    ...
    useEffect(() => {
        if (engine.current.brushCursor) {
            engine.current.brushCursor.visible = transformMode === 'BRUSH';
            const s = brushRadius;
            engine.current.brushCursor.scale.set(s / 0.025, s / 0.025, 1);
        }
    }, [transformMode, brushRadius]);

    // Update island cache when meshes or selection changes
    useEffect(() => {
        if (transformMode !== 'BRUSH') {
            transformEngine.current.updateIslandCache(meshes, selectedIds);
        }
    }, [selectedIds, meshes, version, transformMode]);

    // Interaction State
    const raycaster = useRef(new THREE.Raycaster());
    const mouse = useRef(new THREE.Vector2());

    const screenToUV = (clientX: number, clientY: number): THREE.Vector2 => {
        if (!mountRef.current || !engine.current.camera) return new THREE.Vector2();
        ...
        const wx = mouse.current.x * (camera.right - camera.left) / 2 + (camera.right + camera.left) / 2;
        const wy = mouse.current.y * (camera.top - camera.bottom) / 2 + (camera.top + camera.bottom) / 2;
    ...
    const handlePointerDown = (e: React.PointerEvent) => {
        ...
        const uvPos = screenToUV(e.clientX, e.clientY);
        ...
        if (transformMode === 'BRUSH') {
            ...
            transformEngine.current.handlePointerDown(meshes, selectedIds, uvPos, multiSelect);
            updateUVLines();
        }
    };
    ...
    const handlePointerUp = () => {
        ...
        if (transformMode === 'BRUSH') {
            ...
            transformEngine.current.handlePointerUp();
        }
    ...
    const handlePointerMove = (e: React.PointerEvent) => {
        ...
        if (transformMode === 'BRUSH' && isPointerDown.current) {
            brushEngine.current.applyBrush(
                meshes, selectedIds, engine.current.camera,
                mouse.current,
                { radius: brushRadius, intensity: brushIntensity, type: brushType },
            ...
            updateUVLines();
        } else if (transformMode !== 'BRUSH' && isPointerDown.current) {
            const uvPos = screenToUV(e.clientX, e.clientY);
            transformEngine.current.handlePointerMove(meshes, uvPos);
            updateUVLines();
        }
    };
    ...
    const transformInfo = transformEngine.current.getTransformInfo();
    const selectedIslands = transformEngine.current.getSelectedIslands();

    return (
        <div className="w-full h-full relative bg-[#111] overflow-hidden">
            <div
                ref={mountRef}
                className={`absolute inset-0 ${isPanning.current ? 'cursor-grabbing' : transformMode === 'BRUSH' ? 'cursor-none' : 'cursor-crosshair'}`}
            ...
            <div className="absolute bottom-2 left-2 right-2 flex justify-between items-end pointer-events-none">
                <div className="text-[9px] text-gray-500 font-mono pointer-events-auto flex flex-col gap-0.5">
                    <div>UV EDITOR - {transformMode} MODE</div>
                    {transformMode !== 'SELECT' && selectedIslands.length > 0 && (
                        <div className="text-[8px] text-teal-400">
                            {selectedIslands.length} island{selectedIslands.length > 1 ? 's' : ''} selected
                            {transformMode === 'ROTATE' && ` | Angle: ${transformInfo.angle.toFixed(1)}°`}
                            {transformMode === 'SCALE' && ` | Scale: ${transformInfo.scaleX.toFixed(2)}x, ${transformInfo.scaleY.toFixed(2)}x`}
                        </div>
                    )}
                ...
                <div className="flex gap-2 pointer-events-auto bg-[#1a1a1a] p-1 rounded-md border border-[#333]">
                    ...
                    <button
                        onClick={() => { setTransformMode('SELECT'); transformEngine.current.setMode('SELECT'); }}
                        className={`p-1.5 rounded ${transformMode === 'SELECT' ? 'bg-teal-600 text-white' : 'bg-[#222] text-gray-400'}`}
                        title="Select Islands (S)"
                    >
                        <MousePointer2 size={14} />
                    </button>

                    <button
                        onClick={() => { setTransformMode('MOVE'); transformEngine.current.setMode('MOVE'); }}
                        className={`p-1.5 rounded ${transformMode === 'MOVE' ? 'bg-teal-600 text-white' : 'bg-[#222] text-gray-400'}`}
                        title="Move Islands (G)"
                    >
                        <Move size={14} />
                    </button>

                    <button
                        onClick={() => { setTransformMode('ROTATE'); transformEngine.current.setMode('ROTATE'); }}
                        className={`p-1.5 rounded ${transformMode === 'ROTATE' ? 'bg-teal-600 text-white' : 'bg-[#222] text-gray-400'}`}
                    ...
                    <button
                        onClick={() => { setTransformMode('SCALE'); transformEngine.current.setMode('SCALE'); }}
                        className={`p-1.5 rounded ${transformMode === 'SCALE' ? 'bg-teal-600 text-white' : 'bg-[#222] text-gray-400'}`}
                        title="Scale Islands (S)"
                    >
                        <Maximize size={14} />
                    </button>
                    ...
                    <button
                        onClick={() => { setTransformMode('BRUSH'); transformEngine.current.setMode('BRUSH'); }}
                        className={`p-1.5 rounded ${transformMode === 'BRUSH' ? 'bg-teal-600 text-white' : 'bg-[#222] text-gray-400'}`}
                        title="Brush Mode (B)"
                    >
                        <Brush size={14} />
                    </button>
