...
export default function KQuantumEngine({
    ...
    appMode, cfdResolution, cfdViscosity, cfdBuoyancy, cfdVorticityConfinement, cfdDissipation,
    cfdFluidType, cfdEmitterRadius, cfdEmitterVelocity, cfdEmitterTemperature,
    engineRef
}: KQuantumEngineProps) {
    ...
    // --- CFD STATE ---
    const cfdSimIdRef = useRef<number | null>(null);
    const cfdActiveRef = useRef(false);
    const lastAppModeRef = useRef(appMode);
    ...
    useEffect(() => {
        ...
        const initRustSim = async () => {
            ...
            const simId = await quantum.create({
            ...
            // Store simulation ID in engine ref for export functionality
            if (engineRef.current) {
                engineRef.current.rustSimId = simId;
            }
    ...
    // --- CFD MODE MANAGEMENT ---
    useEffect(() => {
        const wasCFD = lastAppModeRef.current === 'CFD';
        const nowCFD = appMode === 'CFD';
        lastAppModeRef.current = appMode;

        const initCFD = async () => {
            // Dispose old CFD sim if exists
            if (cfdSimIdRef.current !== null) {
                const { cfdClient } = await import('@/services/cfdClient');
                await cfdClient.dispose(cfdSimIdRef.current);
                cfdSimIdRef.current = null;
            }

            console.log(`[KQuantum] 🌊 Initializing CFD mode: ${cfdFluidType} @ ${cfdResolution}³`);

            // Map fluid type to enum
            const fluidTypeMap: Record<string, 'Smoke' | 'Fire' | 'Liquid' | 'Gas' | 'Viscous'> = {
                'Smoke': 'Smoke',
                'Fire': 'Fire',
                'Liquid': 'Liquid',
                'Gas': 'Gas',
            ...
            const { cfdClient } = await import('@/services/cfdClient');
            const simId = await cfdClient.create({
                resolution: [cfdResolution, cfdResolution, cfdResolution],
                fluid_type: fluidTypeMap[cfdFluidType] || 'Smoke',
                viscosity: cfdViscosity,
                buoyancy: cfdBuoyancy,
                vorticity_confinement: cfdVorticityConfinement,
                dissipation: cfdDissipation,
            });

            cfdSimIdRef.current = simId;
            cfdActiveRef.current = true;
            // Expose cfdSimId on engineRef so KQuantum & EntanglementPanel can read it
            if (engineRef.current) {
                engineRef.current.cfdSimId = simId;
            }
        };

        const disposeCFD = async () => {
            if (cfdSimIdRef.current !== null) {
                console.log('[KQuantum] 🌊 Disposing CFD simulation');
                const { cfdClient } = await import('@/services/cfdClient');
                await cfdClient.dispose(cfdSimIdRef.current);
                cfdSimIdRef.current = null;
                cfdActiveRef.current = false;
            }
        };

        if (nowCFD && !wasCFD) {
            // Switching TO CFD mode
            initCFD();
        } else if (!nowCFD && wasCFD) {
            // Switching FROM CFD mode
            disposeCFD();
        } else if (nowCFD && wasCFD) {
            // CFD params changed - reinit
            initCFD();
        }

        return () => {
            // Cleanup on unmount
            if (cfdActiveRef.current && cfdSimIdRef.current !== null) {
                const { cfdClient } = import('@/services/cfdClient');
                cfdClient.then(c => c.cfdClient.dispose(cfdSimIdRef.current!));
            }
        };
    }, [appMode, cfdResolution, cfdFluidType, cfdViscosity, cfdBuoyancy, cfdVorticityConfinement, cfdDissipation]);
    ...
    useEffect(() => {
        ...
        const engine = {
            ...
            rustSimId: null as number | null, // Store Rust simulation ID for export
        ...
        const animate = () => {
            ...
            const usingCFD = appMode === 'CFD' && cfdActiveRef.current;

            // ═══════════════════════════════════════════════════════════
            // CFD MODE: Step CFD simulation and render volume
            // ═══════════════════════════════════════════════════════════
            if (usingCFD && cfdSimIdRef.current !== null) {
                import('@/services/cfdClient').then(async ({ cfdClient }) => {
                    try {
                        // Step CFD simulation
                        ...
                        // TODO: Implement volume rendering visualization
                        // For now, we'll just log occasionally
                        if (Math.random() < 0.01) {
                            console.log(`[KQuantum] 🌊 CFD step complete, density field size: ${densityField.data.length}`);
                        }

                        // Convert density field to particle positions for visualization
                        // This is a simple approach - sample the density field and create particles where density > threshold
                        const threshold = 0.1;
                        const particles: number[] = [];
                        const res = densityField.resolution;
                        const cellSize = 100.0 / res[0]; // Assuming 100 unit bounds

                        for (let z = 0; z < res[2]; z++) {
                            for (let y = 0; y < res[1]; y++) {
                                for (let x = 0; x < res[0]; x++) {
                                    const idx = x + y * res[0] + z * res[0] * res[1];
                                    const density = densityField.data[idx];
                                    
                                    if (density > threshold) {
                                        // Convert grid coords to world space
                                        const wx = (x - res[0] / 2) * cellSize;
                                        const wy = (y - res[1] / 2) * cellSize;
                                        const wz = (z - res[2] / 2) * cellSize;
                                        
                                        particles.push(wx, wy, wz, density);
                                    }
                                }
                            }
                        }

                        // Create texture from particles
                        if (particles.length > 0) {
                            const sqrtCount = Math.ceil(Math.sqrt(particles.length / 4));
                            const texData = new Float32Array(sqrtCount * sqrtCount * 4);
                            
                            for (let i = 0; i < particles.length && i < texData.length; i++) {
                                texData[i] = particles[i];
                            }

                            if (!r.cfdPosTex || r.cfdPosTex.image.width !== sqrtCount) {
                                r.cfdPosTex = new THREE.DataTexture(texData, sqrtCount, sqrtCount, THREE.RGBAFormat, THREE.FloatType);
                                r.cfdVelTex = new THREE.DataTexture(new Float32Array(sqrtCount * sqrtCount * 4), sqrtCount, sqrtCount, THREE.RGBAFormat, THREE.FloatType);
                            } else {
                                r.cfdPosTex.image.data.set(texData);
                            }
                            
                            r.cfdPosTex.needsUpdate = true;
                            r.cfdVelTex.needsUpdate = true;

                            // Use CFD textures for rendering
                            r.mats.render.uniforms.positionTexture.value = r.cfdPosTex;
                            r.mats.render.uniforms.velocityTexture.value = r.cfdVelTex;
                        }
                    } catch (e) {
                        if (Math.random() < 0.01) console.error('[KQuantum] 🌊 CFD step error:', e);
                    }
                });
            } else if (usingRust) {
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        ...
        // CFD MODE: Add emitter on middle-click
        if (appMode === 'CFD' && e.buttons === 4 && cfdSimIdRef.current !== null) {
            import('@/services/cfdClient').then(async ({ cfdClient }) => {
                try {
                    // Convert screen coords to world space (approximate center of view)
                    const worldX = r.mouse.x * 50;
                    const worldY = r.mouse.y * 50;
                    const worldZ = 0;

                    await cfdClient.addSource(
                        cfdSimIdRef.current!,
                        [worldX, worldY, worldZ],
                        cfdEmitterRadius,
                        [0, cfdEmitterVelocity, 0], // Upward velocity
                        1.0, // density
                        cfdEmitterTemperature
                    );
                } catch (e) {
                    console.error('[KQuantum] 🌊 CFD add source error:', e);
                }
            });
        }
    };
