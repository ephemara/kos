...
import { DockPanel, DockTab } from '@/ui/shell/DockPanel';
...
// ─── Viewer Configuration (data-driven — change values here, nowhere else) ───
const VIEWER_CONFIG = {
    camera: {
        fov: 45,
        near: 0.01,
        far: 1000,
        defaultDistance: 3.5,   // fallback distance before fit-to-mesh
        minDistance: 0.3,       // closest zoom
        maxDistance: 50,        // furthest zoom
        fitPadding: 2.2,        // multiplier on bounding sphere radius for fit
    },
    controls: {
        autoRotateDefault: false,   // OFF by default
        autoRotateSpeed: 0.0015,    // radians/frame when auto-rotate is on
        dragSensitivity: 0.005,
        zoomSpeed: 0.005,           // fraction of distance per scroll pixel
        smoothZoom: true,
    },
} as const;

/** Fit camera to completely frame a Three.js object */
function fitCameraToMesh(camera: THREE.PerspectiveCamera, object: THREE.Object3D) {
    ...
    const dist = radius * VIEWER_CONFIG.camera.fitPadding /
        Math.tan((VIEWER_CONFIG.camera.fov / 2) * (Math.PI / 180));
...
export default function KAutopbr({ sharedState, onMaterialCommit, onCommit, tempImage, setTempImage }: any) {
    ...
    // Ref version of autoRotate so animation loop always sees current value (avoids stale closure)
    const autoRotateRef = useRef<boolean>(VIEWER_CONFIG.controls.autoRotateDefault);
    ...
    const [autoRotate, setAutoRotate] = useState<boolean>(VIEWER_CONFIG.controls.autoRotateDefault);
    ...
    const [useNativeRenderer, setUseNativeRenderer] = useState<boolean>(() => {
        ...
        return window.localStorage.getItem('kautopbr.useNativeRenderer') === '1';
    });
    ...
    useEffect(() => {
        ...
        window.localStorage.setItem('kautopbr.useNativeRenderer', useNativeRenderer ? '1' : '0');
    }, [useNativeRenderer]);
    ...
    useEffect(() => {
        ...
        camera.position.set(0, 0, VIEWER_CONFIG.camera.defaultDistance);
        ...
        const animate = () => {
            if (meshRef.current && autoRotateRef.current && !isDragging.current) {
                meshRef.current.rotation.y += VIEWER_CONFIG.controls.autoRotateSpeed;
            }
    ...
    // Keep animation loop ref in sync with autoRotate state (solves stale closure)
    useEffect(() => { autoRotateRef.current = autoRotate; }, [autoRotate]);

    // Mesh & Material Management
    useEffect(() => {
        if (!sceneRef.current || useNativeRenderer) return;
        const scene = sceneRef.current;
        ...
        if (viewShape === 'artifact' && sharedState?.artifact) {
            ...
            loader.load(url, (gltf) => {
                ...
                fitCameraToMesh(cameraRef.current, root);   // ← frame the mesh properly
            ...
            (async () => {
                ...
                fitCameraToMesh(cameraRef.current, newMesh);  // ← frame the primitive
                setSceneMaterials([newMat]); setActiveMaterialId(newMat.uuid);
            })();
        }

        return () => { previewCancelled = true; };
    }, [viewShape, sharedState?.artifact, useNativeRenderer]);
    ...
    }, [maps, activeMaterialId, params, sceneMaterials, useNativeRenderer]);
    ...
    const exportGLB = async () => {
        ...
        exporter.parse(meshRef.current, async (gltf) => {
            ...
            await saveBlobToFile(blob, 'Kipp_Flux_Asset.glb');
        }, (err) => console.error(err), { binary: true, embedImages: true });
    };
    const handleReset = () => {
    ...
    const downloadMap = async (dataUrl: string, name: string) => {
        if (!dataUrl) return;
        const { saveDataUrlToFile } = await import('@/lib/utils/tauriSave');
        await saveDataUrlToFile(dataUrl, `K_${name}.png`);
    };
    // Note: downloadAll opens one dialog per map — user saves each sequentially
    const downloadAll = () => {
        Object.keys(maps).filter(k => maps[k]).forEach(k => downloadMap(maps[k], k));
    };
    ...
    const handlePointerMove = (e) => { if (!isDragging.current || !meshRef.current) return; const delta = { x: e.clientX - previousMousePosition.current.x, y: e.clientY - previousMousePosition.current.y }; meshRef.current.rotation.y += delta.x * VIEWER_CONFIG.controls.dragSensitivity; meshRef.current.rotation.x += delta.y * VIEWER_CONFIG.controls.dragSensitivity; previousMousePosition.current = { x: e.clientX, y: e.clientY }; };
    const handlePointerUp = () => { isDragging.current = false; if (canvasRef.current) canvasRef.current.style.cursor = 'grab'; };
    const handleWheel = (e) => {
        e.preventDefault();
        if (!cameraRef.current) return;
        const cam = cameraRef.current;
        const { minDistance, maxDistance } = VIEWER_CONFIG.camera;
        const { zoomSpeed } = VIEWER_CONFIG.controls;
        // Scale zoom speed proportionally to current distance so it feels consistent
        const delta = e.deltaY * zoomSpeed * cam.position.z;
        const newZ = Math.min(Math.max(cam.position.z + delta, minDistance), maxDistance);
        cam.position.z = newZ;
    };
    ...
    const nativeSyncSource = useMemo<NativeViewportSyncSource>(() => {
        ...
        return { kind: 'primitive', primitiveId: viewShape === 'cube' ? 'cube' : 'sphere' };
    }, [sharedState?.artifact, viewShape]);
    ...
    return (
        ...
        >
            <div className={`flex-1 relative flex flex-col min-w-0 h-full ${useNativeRenderer ? 'bg-transparent pointer-events-none' : 'bg-[#050505]'}`}>
