...
export interface KAlphaToolkitProps {
    alphas: Array<{ name: string, url: string, preview?: string }>;
    activeAlpha: THREE.Texture | null;
    onSelectAlpha: (alpha: THREE.Texture | null) => void;
    onSaveAlpha: (alpha: { name: string, url: string }) => void;
    // Optional callback for "Save as Brush" or extra export features
    onExport?: (type: 'BRUSH' | 'TEXTURE', texture: THREE.Texture, name: string) => void;
    className?: string;
}

export default function KAlphaToolkit({
    alphas,
    activeAlpha,
    onSelectAlpha,
    onSaveAlpha,
    ...
    const handleCommit = () => {
        if (!previewUrl || !onSaveAlpha) return;
        const name = `${genType}_${Date.now().toString().slice(-4)}`;
        onSaveAlpha({ name, url: previewUrl });
        setMode('LIBRARY');
    };

    const handleExportBrush = () => {
        if (!previewUrl || !onExport) return;

        const name = `${genType} Brush`;
        const loader = new THREE.TextureLoader();
        loader.load(previewUrl, (tex) => {
            onExport('BRUSH', tex, name);
        });
    };
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    <div className="grid grid-cols-3 gap-2">
                        <button
                            ...
                            className={`aspect-square rounded-lg border flex flex-col items-center justify-center gap-1 transition-all ${!activeAlpha ? 'bg-rose-500 border-rose-400 text-black' : 'bg-[#111] border-[#333] text-gray-500 hover:border-white'}`}
                        >
                            <Circle size={20} fill={!activeAlpha ? "black" : "none"} />
