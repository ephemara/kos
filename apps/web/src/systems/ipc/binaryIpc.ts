...
export interface BrushResultBinary {
    ...
    new_tangents?: Uint8Array | number[];
    tangent_indices?: Uint8Array | number[];
    time_ms: number;
...
export interface DecodedBrushResult {
    ...
    newTangents?: Float32Array;
    tangentIndices?: Uint32Array;
    timeMs: number;
...
export function decodeBrushResult(result: BrushResultBinary): DecodedBrushResult {
    return {
        ...
        newTangents: result.new_tangents ? bytesToFloat32(result.new_tangents) : undefined,
        tangentIndices: result.tangent_indices ? bytesToUint32(result.tangent_indices) : undefined,
...
export function applyBrushResultToGeometry(
    ...
    // Sparse tangent update for PBR normal mapping
    const tangentAttr = geometry.attributes.tangent;
    if (tangentAttr && result.newTangents && result.tangentIndices) {
        const tangentArray = tangentAttr.array as Float32Array;
        const { tangentIndices, newTangents } = result;
        // Tangents are vec4 (x, y, z, w) where w is handedness
        for (let i = 0; i < tangentIndices.length; i++) {
            const idx = tangentIndices[i];
            tangentArray[idx * 4] = newTangents[i * 4];
            tangentArray[idx * 4 + 1] = newTangents[i * 4 + 1];
            tangentArray[idx * 4 + 2] = newTangents[i * 4 + 2];
            tangentArray[idx * 4 + 3] = newTangents[i * 4 + 3]; // handedness
        }
