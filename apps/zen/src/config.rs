...
fn parse_kain_ui_config(parsed: Option<KainUiConfigFile>) -> KainUiConfig {
    let parsed = parsed.unwrap_or(KainUiConfigFile {
        enabled: true,
        source_path: Some("apps/zen/resources/zen_shell.kn".to_string()),
        modules_manifest_path: Some("apps/zen/resources/modules.toml".to_string()),
        workspace_manifest_path: Some("apps/zen/resources/workspace_ui.toml".to_string()),
        theme_manifest_path: Some("apps/zen/resources/theme.toml".to_string()),
        hot_reload_enabled: Some(true),
        hot_reload_poll_ms: Some(400),
        active_shell: Some("zen.shell.main".to_string()),
        host_api_path: Some("apps/zen/resources/host_api.toml".to_string()),
    ...
    KainUiConfig {
        ...
        modules_manifest_path: parsed
            ...
            .unwrap_or("apps/zen/resources/modules.toml")
            .to_string(),
        workspace_manifest_path: parsed
            .workspace_manifest_path
            .as_deref()
            .filter(|value| !value.trim().is_empty())
            .unwrap_or("apps/zen/resources/workspace_ui.toml")
            .to_string(),
        theme_manifest_path: parsed
            .theme_manifest_path
            .as_deref()
            .filter(|value| !value.trim().is_empty())
            .unwrap_or("apps/zen/resources/theme.toml")
            .to_string(),
        ...
        host_api_path: parsed
            ...
            .as_deref()
            .filter(|value| !value.trim().is_empty())
            .unwrap_or("apps/zen/resources/host_api.toml")
            .to_string(),
