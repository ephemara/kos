 ...
 * # THE GOD WORKFLOW
 * 
 * The `kos.send()` method accepts ANY KosMessage variant - one method to rule them all!
 * 
...
import type { KosMessage } from './KosMessage';
...
export type { KosMessage, KosState, SculptState, BrushLibrary };
...
export const kos = {
    ...
    /**
     * **THE GOD METHOD** - Send ANY KosMessage variant to Bevy
     * 
     ...
     * kos.send({ SetActiveTool: "Sculpt" });
     */
    send: (msg: KosMessage): Promise<void> => invoke('kos_send', { msg }),
    ...
    getState: (): Promise<KosState> => invoke('kos_get_state'),

    /** Get just the sculpt state */
    getSculptState: (): Promise<SculptState> => invoke('kos_get_sculpt_state'),

    /** Get the brush library */
    getBrushLibrary: (): Promise<BrushLibrary> => invoke('kos_get_brush_library'),

    /** Check if Bevy is ready */
    isReady: (): Promise<boolean> => invoke('kos_is_ready'),
    ...
    setActiveTool: (tool: string): Promise<void> => invoke('kos_set_active_tool', { tool }),

    /** Switch to a different brush */
    switchBrush: (brushId: string): Promise<void> => invoke('kos_switch_brush', { brushId }),
    ...
    updateBrushSettings: (settings: {
        ...
        intensity?: number;
        isAdd?: boolean;
    }): Promise<void> => invoke('kos_update_brush_settings', {
    ...
    setSymmetry: (x: boolean, y: boolean, z: boolean): Promise<void> =>
        invoke('kos_set_symmetry', { x, y, z }),

    // --- Geometry Commands ---

    /** Subdivide the current mesh */
    subdivide: (): Promise<void> => invoke('kos_subdivide'),

    /** Remesh the current mesh */
    remesh: (resolution: number): Promise<void> => invoke('kos_remesh', { resolution }),

    /** Undo last action */
    undo: (): Promise<void> => invoke('kos_undo'),

    /** Redo last undone action */
    redo: (): Promise<void> => invoke('kos_redo'),

    // --- Layer Commands ---

    /** Select a layer */
    selectLayer: (entityId: number, addToSelection = false): Promise<void> =>
        invoke('kos_select_layer', { entityId, addToSelection }),

    /** Toggle layer visibility */
    toggleLayerVisibility: (entityId: number): Promise<void> =>
        invoke('kos_toggle_layer_visibility', { entityId }),

    /** Toggle layer lock */
    toggleLayerLock: (entityId: number): Promise<void> =>
        invoke('kos_toggle_layer_lock', { entityId }),

    /** Delete a layer */
    deleteLayer: (entityId: number): Promise<void> =>
        invoke('kos_delete_layer', { entityId }),

    /** Rename a layer */
    renameLayer: (entityId: number, name: string): Promise<void> =>
        invoke('kos_rename_layer', { entityId, name }),

    // --- Viewport Commands ---

    /** Request a full state snapshot from Bevy */
    requestState: (): Promise<void> => invoke('kos_request_state'),

    /** Notify Bevy of window movement */
    windowMove: (x: number, y: number, width: number, height: number): Promise<void> =>
        invoke('kos_window_move', { x, y, width, height }),

    /** Send cursor position */
    cursorMove: (x: number, y: number): Promise<void> =>
        invoke('kos_cursor_move', { x, y }),

    /** Send mouse button event */
    mouseButton: (button: number, pressed: boolean): Promise<void> =>
        invoke('kos_mouse_button', { button, pressed }),

    /** Send scroll event */
    scroll: (deltaX: number, deltaY: number): Promise<void> =>
        invoke('kos_scroll', { deltaX, deltaY }),

    /** Show/hide the transform gizmo (Q key in Bevy, or call this from React) */
    setGizmoVisibility: (visible: boolean): Promise<void> =>
        invoke('kos_send', { msg: { SetGizmoVisibility: { visible } } }),

    // --- Spawn/Import Commands ---

    /** Spawn a primitive */
    spawnPrimitive: (primitiveType: string): Promise<void> =>
        invoke('kos_spawn_primitive', { primitiveType }),

    /** Import a GLTF file */
    importGltf: (path: string): Promise<void> =>
        invoke('kos_import_gltf', { path }),

    // --- System Commands ---

    /** Ping Bevy */
    ping: (): Promise<void> => invoke('kos_ping'),

    /** Shutdown Bevy */
    shutdown: (): Promise<void> => invoke('kos_shutdown'),
};
