...
import { kObjectRegistry, KObject } from '@mocap/three-d/systems/objects/KObjectRegistry';

// ============================================================================
// TYPES
// ============================================================================

type TabId = 'scene' | 'meshes' | 'materials' | 'textures' | 'alphas' | 'takes';

/// Lightweight take summary as returned by the Rust `list_takes` Tauri command.
export interface TakeSummary {
    ...
    duration_ms: number;
    path: string;
    source_type: 'live' | 'video'; // matches TakeSource discriminant
    model_id: string;
    tags: string[];
}
...
interface KContentBrowserProps {
    ...
    alphas: KernelAlpha[];
    takes?: TakeSummary[];           // ZenMocap recorded / analyzed takes
    onOpenTake?: (take: TakeSummary) => void;  // load into timeline/viewport
    onDeleteTake?: (take: TakeSummary) => void;
...
const TABS: { id: TabId; label: string; icon: React.ElementType; color: string }[] = [
    ...
    { id: 'takes', label: 'TAKES', icon: Film, color: 'text-orange-400' },
];
...
export default function KContentBrowser({
    ...
    const getTabContent = () => {
        switch (activeTab) {
            ...
            case 'takes':
                return <TakesGrid items={takes} search={searchQuery} onOpen={onOpenTake} onDelete={onDeleteTake} />;
            default:
                return null;
        }
    };
    ...
    const getTabCount = (tabId: TabId): number => {
        switch (tabId) {
            ...
            case 'takes': return takes.length;
            default: return 0;
        }
    };
...
// ============================================================================
// TAKES GRID — ZenMocap recorded / analyzed animations
// ============================================================================
...
function TakesGrid({ items, search, onOpen, onDelete }: {
    items: TakeSummary[];
    search: string;
    onOpen?: (take: TakeSummary) => void;
    onDelete?: (take: TakeSummary) => void;
}) {
        ...
        return (
            <div className="h-full flex flex-col items-center justify-center text-center">
                <div className="w-16 h-16 rounded-2xl bg-orange-500/10 flex items-center justify-center mb-4">
                    <Film size={32} className="text-orange-500/50" />
                </div>
                <div className="text-sm font-bold text-gray-400 mb-1">No Takes Yet</div>
                <div className="text-xs text-gray-600 max-w-sm">
                    Hit <strong className="text-gray-400">● RECORD</strong> in a live session to capture your first take.
                    <br /><br />
                    <span className="text-gray-500">Takes are saved as <code className="text-orange-400">.zenmocap</code> files and appear here automatically.</span>
                </div>
            </div>
        );
    }
    ...
    if (filtered.length === 0) {
        return (
            <div className="h-full flex items-center justify-center text-xs text-gray-600">
                No takes match &ldquo;{search}&rdquo;
            </div>
        );
    }
    ...
    return (
        <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-3">
            {filtered.map(take => (
                <TakeCard key={take.id} take={take} onOpen={onOpen} onDelete={onDelete} />
            ))}
        </div>
    );
}

function TakeCard({ take, onOpen, onDelete }: {
    take: TakeSummary;
    onOpen?: (take: TakeSummary) => void;
    onDelete?: (take: TakeSummary) => void;
}) {
    const isLive  = take.source_type === 'live';
    const accent  = isLive ? 'border-orange-500/30 hover:border-orange-500/60' : 'border-blue-500/30 hover:border-blue-500/60';
    const badge   = isLive ? 'bg-orange-500/20 text-orange-300' : 'bg-blue-500/20 text-blue-300';
    const icon    = isLive ? '⬤ LIVE' : '▶ VIDEO';

    return (
        <div
            className={`group flex flex-col p-3 rounded-xl bg-[#111] border ${accent} cursor-pointer transition-all hover:scale-[1.02] hover:shadow-lg hover:shadow-orange-500/5`}
            onClick={() => onOpen?.(take)}
        >
            {/* Header row */}
            <div className="flex items-start justify-between mb-2 gap-2">
                <div className="flex-1 min-w-0">
                    <div className="text-[11px] font-bold text-white truncate">{take.name}</div>
                    <div className="text-[9px] text-gray-500 mt-0.5">{formatDate(take.created_at)}</div>
                </div>
                <span className={`text-[8px] font-black px-1.5 py-0.5 rounded flex-shrink-0 ${badge}`}>
                    {icon}
                </span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-1 mb-3">
                <div className="flex flex-col items-center p-1.5 rounded-lg bg-black/30">
                    <Clock size={10} className="text-gray-500 mb-0.5" />
                    <span className="text-[9px] font-bold text-gray-300">{formatDuration(take.duration_ms)}</span>
                </div>
                <div className="flex flex-col items-center p-1.5 rounded-lg bg-black/30">
                    <Film size={10} className="text-gray-500 mb-0.5" />
                    <span className="text-[9px] font-bold text-gray-300">{take.frame_count}f</span>
                </div>
                <div className="flex flex-col items-center p-1.5 rounded-lg bg-black/30">
                    <Cpu size={10} className="text-gray-500 mb-0.5" />
                    <span className="text-[9px] font-bold text-gray-300">{take.fps}fps</span>
                </div>
            </div>

            {/* Model badge */}
            <div className="text-[8px] text-gray-600 truncate mb-3 font-mono">{take.model_id}</div>

            {/* Tags */}
            {take.tags.length > 0 && (
                <div className="flex flex-wrap gap-1 mb-3">
                    {take.tags.slice(0, 3).map(tag => (
                        <span key={tag} className="text-[7px] px-1.5 py-0.5 rounded bg-white/5 text-gray-500 font-mono">
                            {tag}
                        </span>
                    ))}
                </div>
            ...
            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity mt-auto">
                <button
                    className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded-lg bg-orange-500/20 hover:bg-orange-500/40 text-orange-300 text-[9px] font-bold transition-colors"
                    onClick={(e) => { e.stopPropagation(); onOpen?.(take); }}
                >
                    <Play size={10} /> LOAD
                </button>
                <button
