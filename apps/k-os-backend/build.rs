...
fn main() {
    // 1. Get Project Root (we are in apps/k-os-backend, so go up two levels)
...
fn export_specta_tauri_bindings(root: &Path) {
    ...
    let generated_dir = root.join("apps").join("frontend").join("src").join("generated");
    let generated_file = generated_dir.join("tauriRegistry.gen.ts");

    fs::create_dir_all(&generated_dir).expect("Failed to create apps/frontend/src/generated");

    let mut output = String::new();
    output.push_str("// AUTO-GENERATED BY apps/k-os-backend/build.rs. DO NOT EDIT.\n");
...
fn check_directory_sync(root: &Path, tauri_dir: &Path) {
    ...
    // Check Rust modules in apps/k-os-backend/src/
    ...
    // Check React apps in apps/frontend/src/apps/
    let apps_dir = root.join("apps").join("frontend").join("src").join("apps");
    if apps_dir.exists() {
