...
/// Validate all KAIN sculpting shaders in the given directory
pub fn validate_all_shaders(sculpting_dir: &Path) -> Vec<ValidationResult> {
    let mut shader_paths: Vec<PathBuf> = match fs::read_dir(sculpting_dir) {
        Ok(entries) => entries
            .flatten()
...
mod tests {
    ...
    fn test_validate_all_kain_shaders() {
        let sculpting_dir = generated_spv_dir(KainDomain::Sculpting);

        let results = validate_all_shaders(&sculpting_dir);
        ...
        // Assert we validated exactly 17 shaders
        assert_eq!(results.len(), 17, "Expected 17 shaders, found {}", results.len());
    }
}
