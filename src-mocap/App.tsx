...
import { ALL_MODULES, WORKFLOW, CATEGORY_CONFIG } from '@mocap/config/appConfig';
import KernelLauncher, { LaunchConfig } from './features/ZenMocapLauncher/KernelLauncher';
import type { ZenMocapLaunchConfig } from './features/ZenMocap/ZenMocap';
import AssetBrowser from './components/AssetBrowser';
import QuickMenu from '@mocap/shared/shell/QuickMenu';
import { AppTopBar as KOSTopBar } from '@mocap/shared/shell/AppTopBar';
import { AppTopBarDirect as KOSTopBarDirect } from '@mocap/shared/shell/AppTopBarDirect';
import { useQuickMenuCommands } from '@mocap/shared/shell/quickMenuRegistry';
import { useGlobalHotkeys } from '@mocap/lib/hooks/useGlobalHotkeys';
import { WebViewport } from '@mocap/three-d/systems/viewport/WebViewport';
import { assetManager } from '@mocap/shared/systems/assets';
import { registerDefaultAssets } from '@mocap/shared/systems/assets/defaultAssets';
...
import {
    ...
    Terminal, Command, Zap, Database, LayoutGrid, PenTool, Layers, Grid, Copy
} from 'lucide-react';

import { ThemeProvider, ThemeSelector, useTheme } from '@mocap/shared/theme';
import { topBarThemes } from '@mocap/shared/theme/topBarThemes';
import { topBarLayouts } from '@mocap/shared/theme/topBarLayoutConfig';
...
export default function App() {
    ...
    const [zenMocapLaunchConfig, setZenMocapLaunchConfig] = React.useState<ZenMocapLaunchConfig>({});
    const [webcamOpen, setWebcamOpen] = React.useState(false);
    const registeredQuickMenuCommands = useQuickMenuCommands();

    // Debug webcamOpen state changes
    React.useEffect(() => {
        console.log('[App.tsx] webcamOpen state changed to:', webcamOpen);
    }, [webcamOpen]);

    const quickMenuEnabled = !settings.showLauncher;

    // Initialize asset manager on app start
    React.useEffect(() => {
        async function initAssets() {
            try {
                // Register default assets first
                registerDefaultAssets();

                // Then initialize asset manager (will scan directories)
                await assetManager.initialize();
            } catch (error) {
                console.error('[App] Failed to initialize asset manager:', error);
            }
        }
    ...
    const handleLaunchComplete = React.useCallback((config: LaunchConfig) => {
        // Set viewport mode from launcher selection (force simple since Bevy is gone)
        settings.setViewportMode('simple');
        setZenMocapLaunchConfig({
        ...
        settings.switchModule('zen-mocap');

        // Handle project type
        if (config.projectType === 'new') {
            kernel.handleNewProject();
        }
        // TODO: Handle project loading and recent projects
    ...
    return (
        <ThemeProvider appId={settings.activeModuleId || 'default'}>
            ...
            <div className="w-screen h-screen text-[color:var(--kos-text-primary)] font-mono overflow-hidden flex flex-col relative bg-[color:var(--kos-surface-primary)] kos-theme-transition">
                ...
                {settings.topBarLayout === 'direct' ? (
                    ...
                    <KOSTopBarDirect
                        ...
                        onSaveProject={kernel.handleSaveProject}
                        onSwitchProject={() => settings.setShowLauncher(true)}
                        onToggleCameraPreview={() => {
                            console.log('[App.tsx] onToggleCameraPreview handler called');
                            console.log('[App.tsx] webcamOpen before toggle:', webcamOpen);
                            setWebcamOpen(v => {
                                console.log('[App.tsx] setWebcamOpen: old value =', v, ', new value =', !v);
                                return !v;
                            });
                        }}
                        cameraPreviewOpen={webcamOpen}
                    />
                ) : (
                ...
                <main className="flex-1 relative bg-transparent overflow-hidden">
                    {ALL_MODULES.map(mod => {
                        ...
                        let Component: React.ElementType = mod.entryComponent as React.ElementType;

                        console.log('[App.tsx] Rendering module:', mod.id, 'isZenMocap:', mod.id === 'zen-mocap', 'webcamOpen:', webcamOpen);
                            ...
                            >
                                <Component
                                    ...
                                    {...(mod.id === 'zen-mocap' ? {
                                        ...
                                        webcamOpen,
                                        onWebcamClose: () => {
                                            console.log('[App.tsx] onWebcamClose called from ZenMocap');
                                            setWebcamOpen(false);
                                        },
                                    } : {})}
                                />
...
import { useContentBrowser } from '@mocap/shared/systems/content-browser/useContentBrowser';
import { KObject, kObjectRegistry } from '@mocap/three-d/systems/objects/KObjectRegistry';
