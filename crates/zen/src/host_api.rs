...
const DEFAULT_ZEN_HOST_API: &str = include_str!("../resources/host_api.toml");

#[derive(Clone, Debug)]
pub(crate) struct ZenHostApi {
    name: String,
    version: String,
    source_label: String,
...
#[derive(Clone, Debug)]
pub(crate) struct ZenHostAction {
    pub key: String,
    pub label: String,
...
#[derive(Clone, Debug)]
pub(crate) struct ZenHostBinding {
    pub key: String,
    pub label: String,
    pub widget: String,
    pub description: String,
...
pub(crate) enum ZenHostBindingKind {
    SceneObjects,
    SceneSelection,
    SceneViewport,
    EngineRuntime,
...
impl ZenHostApi {
    pub(crate) fn load(configured: &str) -> Result<Self, String> {
    ...
    pub(crate) fn action(&self, key: &str) -> Option<&ZenHostAction> {
        self.actions
            .iter()
            .find(|action| action.key.eq_ignore_ascii_case(key))
    }

    pub(crate) fn binding(&self, key: &str) -> Option<&ZenHostBinding> {
        self.bindings
            .iter()
            .find(|binding| binding.key.eq_ignore_ascii_case(key))
    }

    pub(crate) fn actions(&self) -> &[ZenHostAction] {
        &self.actions
    }

    pub(crate) fn bindings(&self) -> &[ZenHostBinding] {
        &self.bindings
    }
}
...
mod tests {
    ...
    fn parses_default_host_api_manifest() {
        ...
        assert_eq!(api.actions.len(), 10);
        assert!(api.action("scene.spawn_box").is_some());
        assert!(matches!(
            api.action("runtime.play").map(|action| &action.command),
            Some(ZenCommand::RuntimeSetPlayMode {
                mode: ZenPlayMode::Play
            })
        ));
